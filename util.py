...
def evaluate(gts, probabilities, pathologies, use_only_index = None,plroc=False):
    assert(np.all(probabilities >= 0) == True)
    assert(np.all(probabilities <= 1) == True)


    print (gts,probabilities)
    def compute_metrics_for_class(i):
         #print ("true labels and probability")
         #print (gts[:, i],probabilities[:, i])
         print ("=======1")
    ...
    for i in classes:
        ...
        if plroc :
            plot_roc(gts[:,i],probabilities[:, i],pathologies[i])
...
def plot_roc(y_true,y_score,class_name):
    ...
    # Plot ROC curve
    plt.plot(fpr, tpr, label='ROC curve (area = %0.3f)' % roc_auc,linewidth=3.0)
    plt.plot([0, 1], [0, 1], 'k--',linewidth=2.0)  # random predictions curve
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.0])
    plt.xlabel('False Positive Rate or (1 - Specifity)',size='x-large')
    plt.ylabel('True Positive Rate or (Sensitivity)',size='x-large')
    plt.title('Receiver Operating Characteristic',size='x-large')
    plt.legend(loc="lower right")
    plt.savefig('learning_curve_{}.png'.format(class_name))
    plt.close()
